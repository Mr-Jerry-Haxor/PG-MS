...
def dashboard(request):
	...
	if is_pg_admin_dashboard:
		...
		if pg:
			...
			pending = Booking.objects.filter(status=Booking.PENDING, pg=pg).count()
			...
			pending = Booking.objects.filter(status=Booking.PENDING, pg__admins__user=request.user).count()
